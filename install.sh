...
display_help()
{
    ...
    display_message "Manage the installation of libbitcoin-database."
...
{
    display_message "libbitcoin-database installer configuration."
    display_message "--------------------------------------------------------------------"
...
build_all()
{
if [ ! -d "$BUILD_DIR/libbitcoin-database" ]; then
    ...
    build_from_travis JasonCoombs libbitcoin-database master $PARALLEL ${BITCOIN_DATABASE_OPTIONS[@]} "$@"
else
